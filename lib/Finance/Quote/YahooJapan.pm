...
# Maximum number of result pages that a search query can follow.
my $n_pages_per_query = 2;

# Seconds of delay inserted before a request.
my $delay_per_request = 0.5;
...
sub labels {
    return (
        yahoo_japan => [
            qw(method success symbol name date isodate time currency price last nav errormsg)
        ]
    );
}
    ...
    while ( my @syms = splice @symbols, 0, $n_symbols_per_query ) {
        ...
        if ( $reply->is_success ) {
            ...
            for my $sym (@syms) {
                ...
                if ($match) {
                    %info
                        = ( %info, _convert_quote( $quoter, $sym, $match ) );
                }
                elsif ($has_next_page) {
...
sub _convert_quote {
    ...
    $info{ $sym, 'method' }   = 'yahoo_japan';
    $info{ $sym, 'name' }
        = $sym;    # Keep ASCII to avoid JSON serializer crashes
    ...
    if ( !defined $date_str || $date_str !~ /^[0-9]{4}-[0-9]{2}-[0-9]{2}$/ ) {
        my @now = localtime;
        $date_str = sprintf( '%04d-%02d-%02d', $now[5] + 1900, $now[4] + 1,
            $now[3] );
    }
    ...
    eval { $quoter->store_date( \%info, $sym, { isodate => $date_str } ); };
...
sub _scrape {
    ...
    if ( defined $raw_html
        && $raw_html
        =~ m{window\.__PRELOADED_STATE__\s*=\s*(\{.*?\});?\s*</script>}s )
    {
        my $json_text   = $1;
        ...
        if (   $data
            && ref $data eq 'HASH'
            && exists $data->{mainSearchList}{results} )
        {
            for my $item ( @{ $data->{mainSearchList}{results} } ) {
                ...
                my ( $date, $time )
                    = _parse_datetime( $item->{latestPriceTime} // '' );
                my $price = $item->{price} // '';
                $price =~ tr/.0-9//cd;
                next if $price eq '';
                ...
                my $quote = {
                    ...
                    time    => $time,
                    is_fund => (
                        $item->{marketName} && $item->{marketName} =~ /投資信託/
                    ) ? 1 : 0,
                };
    ...
    if ( defined $container ) {
        for my $e ( $container->find('article') ) {
            ...
            my ( $date, $time )
                = $time_elem
                ? _parse_datetime( $time_elem->as_text )
                : ( '', '' );
